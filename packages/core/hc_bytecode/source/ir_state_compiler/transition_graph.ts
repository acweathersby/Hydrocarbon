...
function handleUnresolvedRoots(
    ...
    } /* else if (opt.IS_SCANNER && parent.items.some(i => Sym_Is_A_Generic_Type(i.sym(GRAMMAR)))) {

        console.error("dd", tips.map(t => t.items.map(i => i.renderUnformatted(GRAMMAR))));

        const valid_items = parent.items.filter(i => !Sym_Is_A_Generic_Type(i.sym(GRAMMAR)));
        console.error(valid_items.map(i => i.rup(GRAMMAR)));
        const clone = parent.clone();

        clone.items = valid_items;

        processNode(opt, clone, tpn, true);
